...
type overlap = FullOverlap | PartialOverlap | NoOverlap | Error

let is_full_overlap (e1from, e1to, e2from, e2to) =
  if e1from >= e2from && e1to <= e2to then true
  else if e1from <= e2from && e1to >= e2to then true
  else false

let is_partial_overlap (e1from, e1to, e2from, e2to) =
  if e1from >= e2from && e1from <= e2to then true
  else if e2from >= e1from && e2from <= e1to then true
  else false

let calc e1 e2 =
  match (parse_range e1, parse_range e2) with
  | Some (e1from, e1to), Some (e2from, e2to) ->
      if is_full_overlap (e1from, e1to, e2from, e2to) then FullOverlap
