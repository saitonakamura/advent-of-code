...
let sum_map f acc curr = acc + f curr
...
let parse (r1, r2, g) str =
  ...
  let s = Hash_set.fold ~init:0 ~f inter in
  let r2, g =
    match g with
    | [] -> (r2, [ str ])
    | [ _ ] -> (r2, str :: g)
    | [ l1; l2 ] ->
        let inter2 = find3 l1 l2 str in
        let s2 = Hash_set.fold ~init:0 ~f inter2 in
        (r2 + s2, [])
    | _ :: _ :: _ -> (r2, g)
  in
  (r1 + s, r2, g)
...
let read =
  ...
  let r1, r2, _ =
    List.fold ~init:(0, 0, []) ~f:parse (In_channel.input_lines file)
  in
