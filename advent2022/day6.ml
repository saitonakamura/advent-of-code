...
type 'a fold_window_until_t = Stop of 'a | Continue of 'a
...
let fold_window4_until ~init ~f ~finish l =
  ...
  in
  match loop l (Continue init) with Continue v -> finish v | Stop v -> v
...
let fold _ line =
  let r1 =
    String.to_list line
    |> fold_window4_until ~init:4
         ~finish:(fun _ -> 0)
         ~f:(fun r1 (c1, c2, c3, c4) ->
           if char_different4 c1 c2 c3 c4 then Stop r1 else Continue (r1 + 1))
  in
  let r2 =
    String.to_list line
    |> fold_window14_until ~init:14
         ~finish:(fun _ -> 0)
         ~f:(fun r2 lc ->
           if chars_different lc then Stop r2 else Continue (r2 + 1))
  in
  (r1, r2)
