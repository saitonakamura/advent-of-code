...
let parse_boxes boxes strs = List.fold ~init:boxes ~f:fold strs
...
let parse_instruction str =
  ...
  if Str.string_match r str 0 then
    ...
    { howmany; takefrom = takefrom - 1; moveto = moveto - 1 }
  else raise Fail
...
let parse_line str =
  if String.contains str '[' then StackLine str
  else if String.is_empty str then Empty
    ...
    let count =
      ...
      |> List.map ~f:Int.of_string |> List.fold ~init:0 ~f:max
    in
    StackNumbers count
  else Empty
...
let fold state str =
  ...
  let state =
    match (state, parse_line str) with
    | Start, StackLine str ->
    ...
    | EmptyLine boxes, Instruction instruction ->
        printf "emptyline, instruction: %i %i %i \n" instruction.howmany
          instruction.takefrom instruction.moveto;
...
let parse channel =
  ...
  let r1 =
    ...
    | Instructions boxes ->
        Array.fold ~init:[]
          ~f:(fun l s -> match s with h :: _ -> h :: l | _ -> l)
          boxes
